"""Lookback option payoff functions.

Implements the fixed-strike lookback call payoff based on the running
maximum of the underlying over all T+1 observed prices.
"""
...
def lookback_call_payoff(paths_S: torch.Tensor, K: float) -> torch.Tensor:
    """Fixed-strike lookback call payoff: max(max(S_0..S_T) - K, 0).
    ...
    path_max = paths_S.max(dim=1).values     # (N,)
    return torch.clamp(path_max - K, min=0.0)
